...
namespace GeoShip
{
	...
	{
		if(!CCParticleSystemQuad::initWithTotalParticles(200) )  return false;
	...
	void EmitterExtends::update(float delta)
	{
        ...
        cocos2d::CCLayer* pp = (cocos2d::CCLayer*)getParent();
        
        if(pp != NULL)
        {
            m_border.origin.x = pp->getPositionX();
            m_border.origin.y = pp->getPositionY();
        }
